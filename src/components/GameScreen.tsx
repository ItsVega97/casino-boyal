...
export const GameScreen: React.FC<GameScreenProps> = ({ onGameOver }) => {
  ...
  if (state.ui.screen !== 'game' && state.ui.screen !== 'shop') {
    console.error('GameScreen: Unknown screen state', {
    ...
    return (
      <div className="min-h-dvh w-full bg-black flex items-center justify-center p-4">
        <div className="bg-gradient-to-b from-red-950 to-black border-4 border-red-600 rounded-2xl shadow-2xl max-w-2xl w-full p-8">
          <div className="text-center">
            <p className="text-red-400 text-3xl font-black mb-4">ERROR: PANTALLA DESCONOCIDA</p>
            <div className="bg-black/50 border-2 border-red-800 rounded-lg p-4 mb-6">
              <p className="text-red-300 mb-2">
                <span className="font-bold">Screen:</span> {String(state.ui.screen)}
              </p>
              <p className="text-red-300 mb-2">
                <span className="font-bold">Phase:</span> {state.phase}
              </p>
              <p className="text-red-300">
                <span className="font-bold">Round:</span> {state.round}
              </p>
            </div>
            <p className="text-gray-400 mb-6">Este es un estado inválido. Por favor reporta este error.</p>
            <button
              onClick={() => dispatch({ type: 'GO_TO_MENU' })}
  ...
  return (
    <div className="min-h-dvh w-full bg-black">
      ...
      {import.meta.env.DEV && (
        <div className="fixed bottom-2 left-2 bg-black/90 border-2 border-yellow-500 p-3 text-xs font-mono rounded z-[100] max-w-xs">
          <div className="font-bold mb-2 text-green-400 border-b border-green-700 pb-1">DEBUG HUD</div>
          <div className="space-y-1 text-gray-300">
            <div><span className="text-yellow-500">screen:</span> <span className="text-white font-bold">{state.ui.screen}</span></div>
            <div><span className="text-yellow-500">phase:</span> <span className="text-white font-bold">{state.phase}</span></div>
            <div><span className="text-yellow-500">round:</span> <span className="text-white font-bold">{state.round}</span></div>
            <div><span className="text-yellow-500">tickets:</span> <span className="text-white font-bold">{state.tickets}</span></div>
            <div><span className="text-yellow-500">shopOffers:</span> <span className="text-white font-bold">{Array.isArray(state.shopOffers) ? state.shopOffers.length : 'INVALID'}</span></div>
            <div><span className="text-yellow-500">ownedUpgrades:</span> <span className="text-white font-bold">{Array.isArray(state.ownedUpgrades) ? state.ownedUpgrades.length : 'INVALID'}</span></div>
            <div><span className="text-yellow-500">chips:</span> <span className="text-white font-bold">{state.chips}</span></div>
            <div><span className="text-yellow-500">spins:</span> <span className="text-white font-bold">{state.spinsLeft}</span></div>
          </div>
        </div>
      )}
    </div>
